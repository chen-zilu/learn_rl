...
class DDQN:
    ...
    def update(self):
        if len(self.exp_pool) >= self.minimal_size:
            ...
            ac1 = self.q_net(exp_state_nx).argmax(1)
            q_max_new = self.q_tar_net(exp_state_nx).gather(1, ac1.view(-1, 1)).squeeze(1).detach()

            loss = torch.mean((exp_reward + self.gamma * q_max_new * (1-exp_done) - q_out_all)**2)
...
def main():
        ...
        with tqdm(total=int(num_episodes / 10), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes/10)):
                ...
                if episode_return > max_reward:
                    ...
                    torch.save(agent.q_net.state_dict(), "best_ddqn_q_net_cartpole.pth")
                if (i_episode + 1) % 10 == 0:
                    pbar.set_postfix({
                        'episode':
    ...
    torch.save(agent.q_net.state_dict(), "ddqn_q_net_cartpole.pth")


# 修正后的测试代码
def test_agent(model_path="true_dqn_q_net_cartpole.pth"):
    # 创建测试环境，设置渲染模式
...
if __name__ == '__main__':
    # main()
    test_agent("best_ddqn_q_net_cartpole.pth")
